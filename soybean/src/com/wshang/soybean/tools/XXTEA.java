...
public final class XXTEA 
{
	...
	//解密 处理后base_64
	public static String  decode(String encode,String key)
	{
		byte[] b    = android.util.Base64.decode(encode,Base64.DEFAULT);
		return xxtea_decrypt(b,key);
	}

	//加密字符串转成base64
	public static String  encode(String encode,String key)
	{
		byte[] b = xxtea_encrypt(encode,key);
		return byteToString(b);
	}
	
	public static String byteToString(byte[] encode)
	{
		return android.util.Base64.encodeToString(encode,Base64.DEFAULT);
	}
	...
	public static byte[] xxtea_encrypt(String data,String key)
	{
		...
		return XXTEA.encrypt(a,b);//加密
	}

	//xxtea解密
	public static String xxtea_decrypt(byte[] data,String key)
	{
		byte[] b = null;
		try {
			b = key.getBytes("UTF-8");
		...
		return new String(XXTEA.decrypt(data,b));
	}
